...
| <a name="input_bucket_prefix"></a> [bucket\_prefix](#input\_bucket\_prefix) | Bucket prefix, which will include a randomised suffix to ensure globally unique names when bucket_namespace is 'global', or a region and account-specific suffix when bucket_namespace is 'account-regional'. | `string` | `null` | no |
